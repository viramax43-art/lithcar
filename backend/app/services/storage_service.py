...
def upload_driver_application_file(*, filename: str, content_type: str, content: bytes) -> str:
    key = f"driver-applications/{uuid4().hex}_{filename}"
    client = _build_s3_client()
    client.put_object(
        Bucket=settings.s3_bucket_name,
        Key=key,
...
def download_driver_application_file(*, object_key: str) -> tuple[bytes, str]:
    client = _build_s3_client()
    response = client.get_object(Bucket=settings.s3_bucket_name, Key=object_key)
    content_type = response.get("ContentType", "application/octet-stream")
    body = response["Body"].read()
