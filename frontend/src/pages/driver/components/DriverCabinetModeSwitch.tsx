...
export default function DriverCabinetModeSwitch({
  ...
  const btnCls = (active: boolean) =>
    `flex-1 min-h-10 rounded-xl text-xs font-bold transition-colors touch-none ${
      active ? 'bg-black text-white' : 'text-muted hover:bg-surface'
    }`

  return (
    <div
      className="absolute left-3 right-3 z-[12] bg-white rounded-card shadow-card p-1 flex gap-1"
